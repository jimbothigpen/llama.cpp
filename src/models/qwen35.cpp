...
void llama_model_qwen35::load_arch_hparams(llama_model_loader & ml) {
    ...
    // MTP GGUFs embed `nextn_predict_layers` trailing full-attention blocks plus
    // per-block `nextn.*` tensors (eh_proj/enorm/hnorm/shared_head_*). The MTP
    // head graph is dispatched via LLM_GRAPH_TYPE_DECODER_MTP from build_arch_graph.
...
void llama_model_qwen35::load_arch_tensors(llama_model_loader &) {
    ...
    for (int i = 0; i < n_layer; ++i) {
        ...
        // Load NEXTN_* per-block tensors for the trailing nextn_predict_layers MTP blocks.
        // The trunk graph ignores them; they are required so the GGUF loads successfully
        // and are consumed by graph_mtp when ctx_type == MTP.
        const uint32_t n_transformer_layers = hparams.n_layer - hparams.nextn_predict_layers;
...
llama_model_qwen35::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    // LIFT-WITH-GLUE-REAUTHOR from buun SD-091 bf22e115e src/models/qwen35.cpp lines 235-274.
    // Runs inside ctx_dft graph (graph_mtp), not inline in main graph as in buun.
