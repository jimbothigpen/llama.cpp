...
llama_context::llama_context(
    ...
    if (cparams.ctx_type == LLAMA_CONTEXT_TYPE_MTP && model.hparams.n_layer_nextn == 0 &&
...
void llama_context::set_diffusion_self_cond(const float * probs, int64_t n_vocab, int64_t n_tokens) {
    if (probs == nullptr || n_vocab <= 0 || n_tokens <= 0) {
        // clear -> zero self-conditioning (step 1 / encoder phase)
        diffusion_cond.enabled  = false;
...
// llama_set_eagle3_g_embeddings) was retired when mainline's in-graph EAGLE3 (#18039) was adopted
// (Path A forward-sync 2026-06-13). The mainline EAGLE3 extraction API lives in llama-ext.cpp
// (llama_{set,get}_embeddings_layer_inp / llama_{set,get}_embeddings_nextn).
