...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    switch (gating_op) {
        ...
        case LLAMA_EXPERT_GATING_FUNC_TYPE_NONE:
            {
                probs = logits; // already-normalized expert probabilities supplied via probs_in
            } break;
        default:
            GGML_ABORT("fatal error");
    }
...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        if (v->type == GGML_TYPE_TURBOQ2_0 || v->type == GGML_TYPE_TURBOQ3_0 || v->type == GGML_TYPE_TURBOQ4_0) {
            if (cur->ne[0] % 128 == 0) {
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (k->type == GGML_TYPE_TURBOQ2_0 || k->type == GGML_TYPE_TURBOQ3_0 || k->type == GGML_TYPE_TURBOQ4_0) {
