...
struct common_speculative_impl_draft_dflash : public common_speculative_impl {
    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            const llama_pos n = llama_memory_seq_pos_max(llama_get_memory(ctx_dft), seq_id) + 1;
...
common_speculative * common_speculative_init(common_params_speculative & params, uint32_t n_seq) {
    ...
    std::vector<common_speculative_config> configs = {}; // list of speculative configs to try
    {
        ...
        // mainline-native DFlash (KV-injection, block-masked; PR #22105) — separate arch/CLI
        // token from our COMMON_SPECULATIVE_TYPE_DFLASH (cross-attention-ring), see
        // docs/features/dflash.md disambiguation note.
        bool has_draft_dflash       = (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH)) && params.draft.ctx_dft != nullptr;
        bool requested_draft_dflash = (enabled_configs & (1u << COMMON_SPECULATIVE_TYPE_DRAFT_DFLASH));
...
void common_speculative_print_stats(const common_speculative * spec) {
    ...
    for (const auto & impl : spec->impls) {
        ...
        if (impl->n_call_accept > 0) {
            ...
            str_stats = ", #mean acc len = " + oss.str();
        }
