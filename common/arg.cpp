...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    if (params.model.path.empty() && ctx_arg.ex != LLAMA_EXAMPLE_SERVER && !skip_model_download && !params.usage && !params.completion && !params.list_sidecar_types) {
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    ).set_examples({LLAMA_EXAMPLE_LOOKUP, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        string_format(
            "KV cache data type for K in SWA layers (default: same as --cache-type-k)\n"
            "allowed values: %s",
            get_all_kv_cache_types().c_str()
        ),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        string_format(
            "KV cache data type for V in SWA layers (default: same as --cache-type-v)\n"
            "allowed values: %s",
            get_all_kv_cache_types().c_str()
        ),
        [](common_params & params, const std::string & value) {
    ...
    ).set_spec().set_examples({LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_PERPLEXITY}));
    ...
    add_opt(common_arg(
        {"--pflash-cache-dir"}, "PATH",
        "PFlash: directory for on-disk scorer cache (default: same directory as scorer model). "
        "Cache is invalidated automatically when the scorer model path or mtime changes.",
        [](common_params & params, const std::string & value) {
            params.speculative.pflash_scorer_cache_dir = value;
        }
    ).set_spec().set_examples({LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_PERPLEXITY}));
