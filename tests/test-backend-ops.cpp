...
struct test_set_rows : public test_case {
    ...
    double max_nmse_err() override {
        ...
        if (type == GGML_TYPE_WHT4_0) {
            // Reduction order matters; WHT4_0 has 32-element WHT inside the
...
struct test_turbo_wht : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        ggml_set_param(a);
        ggml_set_name(a, "a");
        ggml_tensor * out = ggml_turbo_wht(ctx, a, direction);
        ggml_set_name(out, "out");
        return out;
    }
};
...
struct test_turbo_wht_roundtrip : public test_case {
    ...
    ggml_tensor * build_graph(ggml_context * ctx) override {
        ...
        ggml_tensor * fwd = ggml_turbo_wht(ctx, a, 0);
        ggml_tensor * inv = ggml_turbo_wht(ctx, fwd, 1);
        ggml_set_name(inv, "out");
        return inv;
    }
};

// Test SET_ROWS with turboq3 destination, then dequantize and compare.
// This validates the full quantization pipeline: f32 -> WHT -> PolarQuant -> turboq3
// followed by dequantization: turboq3 -> f32. The round-trip error should be bounded.
...
struct test_set_rows_turboq3 : public test_case {
    const ggml_type type_idx;
    ...
    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return "SET_ROWS_TURBOQ3";
    }

    test_set_rows_turboq3(ggml_type type_idx = GGML_TYPE_I32,
            int64_t ne0 = 128, int64_t ne1 = 8, int r = 4)
        : type_idx(type_idx), ne0(ne0), ne1(ne1), r(r) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        // dst: the turboq3 KV cache buffer
        ggml_tensor * dst = ggml_new_tensor_2d(ctx, GGML_TYPE_TURBOQ3_0, ne0, ne1);
        ggml_set_name(dst, "dst");
        ...
        // Write f32 data into turboq3 dst via SET_ROWS (includes WHT + quantize)
    ...
    double max_nmse_err() override {
        // turboq3 is 3-bit quantization with WHT rotation.
        // The round-trip error (f32 -> turboq3 -> f32) is higher than q8_0
...
struct test_set_rows_wht4_0 : public test_case {
    const ggml_type type_idx;
    ...
    std::string op_desc(ggml_tensor * t) override {
        GGML_UNUSED(t);
        return "SET_ROWS_WHT4_0";
    }

    test_set_rows_wht4_0(ggml_type type_idx = GGML_TYPE_I32,
            int64_t ne0 = 32, int64_t ne1 = 8, int r = 4)
        : type_idx(type_idx), ne0(ne0), ne1(ne1), r(r) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        // dst: the WHT4_0 weight buffer
        ggml_tensor * dst = ggml_new_tensor_2d(ctx, GGML_TYPE_WHT4_0, ne0, ne1);
        ggml_set_name(dst, "dst");
        ...
        // Write f32 data into WHT4_0 dst via SET_ROWS (includes WHT + quantize)
        ggml_tensor * written = ggml_set_rows(ctx, dst, src, row_idxs);
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != NULL; t = ggml_get_next_tensor(ctx, t)) {
            if (t->type == GGML_TYPE_I64 || t->type == GGML_TYPE_I32) {
                if (ggml_is_view_op(t->op)) continue;
                init_set_rows_row_ids(t, ne1);
            } else if (t->type == GGML_TYPE_WHT4_0) {
                // Zero-fill WHT4_0 dst to avoid fp16 NaN in unwritten rows' d0/d1
    ...
    double max_nmse_err() override {
        // GPU and CPU WHT4_0 quantization diverge due to subgroupAdd reduction
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // WHT4_0: Gemma-4 E2B dimensions. The fused mul_mat_vec kernel has a
    ...
    for (int k : { 1536, 2048, 2304, 3072, 4096 }) {
        for (int m : { 256, 1152, 1536, 2048, 5120, 6144 }) {
            for (int n : { 1, 2, 4, 8 }) {
                test_cases.emplace_back(new test_mul_mat(GGML_TYPE_WHT4_0, GGML_TYPE_F32, m, n, k, {1, 1}, {1, 1}));
                test_cases.emplace_back(new test_mul_mat(GGML_TYPE_WHT4_0, GGML_TYPE_F16, m, n, k, {1, 1}, {1, 1}));
            }
        }
    }

    // WHT4_0: large-batch MUL_MAT exercises the dequant + f16 matmul path used
    ...
    // are dequantized via pipeline_dequant[WHT4_0] into a temporary f16 buffer and
    ...
    // SET_ROWS with turboq3 destination: quantize then dequant round-trip
    // Small tensors (single-dim dispatch)
    for (ggml_type idx_type : {GGML_TYPE_I32, GGML_TYPE_I64}) {
        for (int64_t ne0 : {128, 256, 512}) {
            for (int r : {1, 4, 7}) {
                test_cases.emplace_back(new test_set_rows_turboq3(idx_type, ne0, 16, r));
            }
        }
    }
    ...
    for (ggml_type idx_type : {GGML_TYPE_I32, GGML_TYPE_I64}) {
        for (int64_t ne0 : {32, 64, 128, 256}) {
            for (int r : {1, 4}) {
                test_cases.emplace_back(new test_set_rows_wht4_0(idx_type, ne0, 16, r));
            }
        }
    }
    // Large tensor
    test_cases.emplace_back(new test_set_rows_wht4_0(GGML_TYPE_I32, 128, 256, 64));
                                        ...
                                        for (int kv : { 113, 512, 1024, }) {
                                            ...
                                            for (int nb : { 1, 3, 32, 75, }) {
                                                for (ggml_prec prec : {GGML_PREC_F32, GGML_PREC_DEFAULT}) {
                                                    ...
                                                    for (ggml_type type_KV : {GGML_TYPE_F32, GGML_TYPE_F16, GGML_TYPE_BF16, GGML_TYPE_Q8_0, GGML_TYPE_Q5_1, GGML_TYPE_Q5_0, GGML_TYPE_Q4_1, GGML_TYPE_Q4_0, GGML_TYPE_IQ4_NL, GGML_TYPE_TURBOQ3_0}) {
                                                        if (type_KV == GGML_TYPE_TURBOQ3_0 && hsk < 128) continue;
