...
extern "C" {
GGML_API int turboq3_cpu_wht_group_size;
}
...
static void ggml_compute_forward_set_rows_f32(
    ...
    if (dst->type == GGML_TYPE_TURBOQ2_0 || dst->type == GGML_TYPE_TURBOQ3_0 || dst->type == GGML_TYPE_TURBOQ4_0 ||
        dst->type == GGML_TYPE_TURBOQ2_INNERQ || dst->type == GGML_TYPE_TURBOQ3_INNERQ || dst->type == GGML_TYPE_TURBOQ4_INNERQ) {
        int gs = 0;
...
// WHT sign arrays (must match Metal/CUDA shader turbo_wht_signs1/2)
...
// GGML_OP_FWHT standalone op — reads from src[0], writes to dst (same shape)
static void ggml_compute_forward_fwht_op_f32(const ggml_compute_params * params, ggml_tensor * dst) {
    const ggml_tensor * src = dst->src[0];
    ...
    for (int64_t r = start_row; r < end_row; r++) {
        ...
        // Scalar butterfly passes (stages below SIMD width)
#if defined(GGML_SIMD)
        const int step = GGML_F32_EPR;
#else
...
void ggml_compute_forward_fwht_op(const ggml_compute_params * params, ggml_tensor * dst) {
    switch (dst->src[0]->type) {
        case GGML_TYPE_F32:
            ggml_compute_forward_fwht_op_f32(params, dst);
            break;
        default:
            GGML_ABORT("fwht op is F32 only");
    }
}
