...
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_q1_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_q1_0 * K_q1_0 = (const block_q1_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
    for (int k_KQ_0 = 0; k_KQ_0 < D/2; k_KQ_0 += nthreads) {
        const int k_KQ  = k_KQ_0 + (nthreads == WARP_SIZE ? threadIdx.x : threadIdx.x % nthreads);
        const int ib    = (2*k_KQ) / QK1_0;
        const int ibit  = (2*k_KQ) % QK1_0;  // always even
        const float d   = __half2float(K_q1_0[ib].d);
        const uint8_t b = K_q1_0[ib].qs[ibit/8];
        const int shift = ibit % 8;
        const float K0  = ((b >> shift)     & 1) ? d : -d;
        const float K1  = ((b >> (shift+1)) & 1) ? d : -d;
#ifdef V_DOT2_F32_F16_AVAILABLE
        const float2 Q_f = __half22float2(((const half2 *) Q_v)[k_KQ_0/nthreads]);
#else
        const float2 Q_f = ((const float2 *) Q_v)[k_KQ_0/nthreads];
#endif // V_DOT2_F32_F16_AVAILABLE
        sum += K0 * Q_f.x + K1 * Q_f.y;
    }
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_q1_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_q1_0 * x = (const block_q1_0 *) vx;
    const int64_t ib   = i0 / QK1_0;
    const int     ibit = i0 % QK1_0;
    const float   d    = __half2float(x[ib].d);

#ifdef FP16_AVAILABLE
    if constexpr (std::is_same<T, half>::value) {
        static_assert(ne % 2 == 0, "bad ne");
        const half dh = x[ib].d;
#pragma unroll
        for (int l0 = 0; l0 < ne; l0 += 2) {
            const int abs0 = ibit + l0;
            const int abs1 = ibit + l0 + 1;
            const half v0 = ((x[ib].qs[abs0/8] >> (abs0%8)) & 1) ? dh : -dh;
            const half v1 = ((x[ib].qs[abs1/8] >> (abs1%8)) & 1) ? dh : -dh;
...
#pragma unroll
        for (int l = 0; l < ne; ++l) {
            const int abs_bit = ibit + l;
            ((float *) dst)[l] = ((x[ib].qs[abs_bit/8] >> (abs_bit%8)) & 1) ? d : -d;
        }
    } else {
        static_assert(std::is_same_v<T, void>, "unsupported type");
    }
}
// === TurboQuant KQ vec_dot functions (turboq3, turboq4) ===
// TurboQuant3 KQ dot product: dequantize K from turboq3 blocks, dot with Q (float2/half2)
...
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turboq3_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turboq3_0 * K_turbo = (const block_turboq3_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int elem0 = k_KQ * 2;                  // always even
            const int ib    = elem0 / QK_TURBOQ3;          // shared block index
            const int j0    = elem0 % QK_TURBOQ3;          // always even, 0..30
...
// TurboQuant4 KQ dot product: dequantize K from turboq4 blocks, dot with Q (float2/half2)
// 4-bit nibble packed: qs[j/2] >> ((j%2)*4) & 0xF
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turboq4_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {
...
// TurboQuant5 KQ dot product: dequantize K from turboq5 blocks (5-bit uniform grid), dot with Q.
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turboq5_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turboq5_0 * K_turbo = (const block_turboq5_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int elem0 = k_KQ * 2;                   // always even
            const int ib    = elem0 / QK_TURBOQ5;           // block index
            const int j0    = elem0 % QK_TURBOQ5;           // always even

            const float norm = __half2float(K_turbo[ib].norm);

            float2 kv;
            kv.x = turboq5_dequant_element(&K_turbo[ib], j0,     norm);
            kv.y = turboq5_dequant_element(&K_turbo[ib], j0 + 1, norm);
...
// TurboQuant6 KQ dot product: dequantize K from turboq6 blocks (6-bit uniform grid), dot with Q.
template <int D, int nthreads>
static __device__ __forceinline__ float vec_dot_fattn_vec_KQ_turboq6_0(
    const char * __restrict__ K_c, const void * __restrict__ Q_v, const int * __restrict__ Q_q8, const void * __restrict__ Q_ds_v) {

    const block_turboq6_0 * K_turbo = (const block_turboq6_0 *) K_c;
    GGML_UNUSED(Q_q8);
    GGML_UNUSED(Q_ds_v);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int elem0 = k_KQ * 2;                   // always even
            const int ib    = elem0 / QK_TURBOQ6;           // block index
            const int j0    = elem0 % QK_TURBOQ6;           // always even

            const float norm = __half2float(K_turbo[ib].norm);

            float2 kv;
            kv.x = turboq6_dequant_element(&K_turbo[ib], j0,     norm);
            kv.y = turboq6_dequant_element(&K_turbo[ib], j0 + 1, norm);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int elem0 = k_KQ * 2;                   // always even
            const int ib    = elem0 / QK_TURBOQ2_TCQ;     // block index
            const int j0    = elem0 % QK_TURBOQ2_TCQ;     // always even, 0..126

            const float norm = __half2float(K_turbo[ib].norm);

            // Element t at bit position t*2; sliding-window 8-bit state read.
            // For paired decode of elem0 and elem0+1: bit positions 2*j0 and 2*j0+2.
            const int bit0 = j0 * 2;
            const int bit1 = bit0 + 2;
            const int byte_idx0 = bit0 / 8;
            const int bit_off0  = bit0 & 7;
            const int byte_idx1 = bit1 / 8;
            const int bit_off1  = bit1 & 7;

            const uint16_t raw0 = (uint16_t) K_turbo[ib].qs[byte_idx0]
                                | ((uint16_t) K_turbo[ib].qs[byte_idx0 + 1] << 8);
            const uint16_t raw1 = (uint16_t) K_turbo[ib].qs[byte_idx1]
                                | ((uint16_t) K_turbo[ib].qs[byte_idx1 + 1] << 8);

            const int state0 = (raw0 >> bit_off0) & 0xFF;   // 8-bit state
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            const int elem0 = k_KQ * 2;                   // always even
            const int ib    = elem0 / QK_TURBOQ3_TCQ;     // block index
            const int j0    = elem0 % QK_TURBOQ3_TCQ;     // always even, 0..126

            const float norm = __half2float(K_turbo[ib].norm);

            // Element t at bit position t*3; sliding-window 9-bit state read.
            const int bit0 = j0 * 3;
            ...
            const int state0 = (raw0 >> bit_off0) & 0x1FF;   // 9-bit state
...
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turboq3_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turboq3_0 * x = (const block_turboq3_0 *) vx;

    const int64_t ib   = i0 / QK_TURBOQ3;
...
#ifdef FP16_AVAILABLE
        if constexpr (std::is_same_v<T, half>) {
            float v0 = turboq3_dequant_element(&x[ib], j0,   norm);
            float v1 = turboq3_dequant_element(&x[ib], j0+1, norm);
            ((half2 *) dst)[0] = make_half2(__float2half(v0), __float2half(v1));
        } else
#endif // FP16_AVAILABLE
        if constexpr (std::is_same_v<T, float>) {
            ((float *) dst)[0] = turboq3_dequant_element(&x[ib], j0,   norm);
            ((float *) dst)[1] = turboq3_dequant_element(&x[ib], j0+1, norm);
        } else {
            static_assert(std::is_same_v<T, void>, "unsupported type");
        }
    }
...
// TurboQuant4 V dequantize: extract `ne` float/half values at position i0.
// 4-bit nibble packed, block size 128.
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turboq4_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turboq4_0 * x = (const block_turboq4_0 *) vx;

    const int64_t ib   = i0 / QK_TURBOQ4;
...
#ifdef FP16_AVAILABLE
        if constexpr (std::is_same_v<T, half>) {
            float v0 = turboq4_dequant_element(&x[ib], j0,   norm);
            float v1 = turboq4_dequant_element(&x[ib], j0+1, norm);
            ((half2 *) dst)[0] = make_half2(__float2half(v0), __float2half(v1));
        } else
#endif // FP16_AVAILABLE
        if constexpr (std::is_same_v<T, float>) {
            ((float *) dst)[0] = turboq4_dequant_element(&x[ib], j0,   norm);
            ((float *) dst)[1] = turboq4_dequant_element(&x[ib], j0+1, norm);
        } else {
            static_assert(std::is_same_v<T, void>, "unsupported type");
        }
    }
...
// TurboQuant5 V dequantize: extract `ne` float/half values at position i0.
// 5-bit, block size 128. Uniform grid decode via turboq5_dequant_element.
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turboq5_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turboq5_0 * x = (const block_turboq5_0 *) vx;

    const int64_t ib   = i0 / QK_TURBOQ5;
    const int     j0   = i0 % QK_TURBOQ5;
    const float   norm = __half2float(x[ib].norm);

    static_assert(ne == 2 || ne == 4, "bad ne");

    if constexpr (ne == 4) {
        const float v0 = turboq5_dequant_element(&x[ib], j0,   norm);
        const float v1 = turboq5_dequant_element(&x[ib], j0+1, norm);
        const float v2 = turboq5_dequant_element(&x[ib], j0+2, norm);
        const float v3 = turboq5_dequant_element(&x[ib], j0+3, norm);
...
#endif // FP16_AVAILABLE
        if constexpr (std::is_same_v<T, float>) {
            ((float *) dst)[0] = turboq5_dequant_element(&x[ib], j0,   norm);
            ((float *) dst)[1] = turboq5_dequant_element(&x[ib], j0+1, norm);
        } else {
            static_assert(std::is_same_v<T, void>, "unsupported type");
        }
    }
...
// TurboQuant6 V dequantize: extract `ne` float/half values at position i0.
// 6-bit, block size 128. Uniform grid decode via turboq6_dequant_element.
template <typename T, int ne>
static __device__ __forceinline__ void dequantize_V_turboq6_0(const void * __restrict__ vx, void * __restrict__ dst, const int64_t i0) {
    const block_turboq6_0 * x = (const block_turboq6_0 *) vx;

    const int64_t ib   = i0 / QK_TURBOQ6;
    const int     j0   = i0 % QK_TURBOQ6;
    const float   norm = __half2float(x[ib].norm);

    static_assert(ne == 2 || ne == 4, "bad ne");

    if constexpr (ne == 4) {
        const float v0 = turboq6_dequant_element(&x[ib], j0,   norm);
        const float v1 = turboq6_dequant_element(&x[ib], j0+1, norm);
        const float v2 = turboq6_dequant_element(&x[ib], j0+2, norm);
        const float v3 = turboq6_dequant_element(&x[ib], j0+3, norm);
...
#endif // FP16_AVAILABLE
        if constexpr (std::is_same_v<T, float>) {
            ((float *) dst)[0] = turboq6_dequant_element(&x[ib], j0,   norm);
            ((float *) dst)[1] = turboq6_dequant_element(&x[ib], j0+1, norm);
        } else {
            static_assert(std::is_same_v<T, void>, "unsupported type");
        }
    }
...
static __device__ __forceinline__ void dequantize_V_turboq2_tcq_cb(
    ...
    auto decode_at = [&](int j) {
        const int bit_pos  = j * 2;
    ...
    if constexpr (ne == 4) {
        const float v0 = decode_at(j0);
        const float v1 = decode_at(j0 + 1);
        const float v2 = decode_at(j0 + 2);
        const float v3 = decode_at(j0 + 3);
#ifdef FP16_AVAILABLE
        if constexpr (std::is_same_v<T, half>) {
            ((half2 *) dst)[0] = make_half2(__float2half(v0), __float2half(v1));
            ((half2 *) dst)[1] = make_half2(__float2half(v2), __float2half(v3));
...
constexpr __device__ vec_dot_KQ_t get_vec_dot_KQ() {
    ...
    } else if constexpr (type_K == GGML_TYPE_Q1_0) {
        return vec_dot_fattn_vec_KQ_q1_0<D, nthreads>;
    ...
    } else if constexpr (type_K == GGML_TYPE_TURBOQ5_0) {
        return vec_dot_fattn_vec_KQ_turboq5_0<D, nthreads>;
    } else if constexpr (type_K == GGML_TYPE_TURBOQ6_0) {
        return vec_dot_fattn_vec_KQ_turboq6_0<D, nthreads>;
...
constexpr __device__ dequantize_V_t get_dequantize_V() {
    ...
    } else if constexpr (type_V == GGML_TYPE_Q1_0) {
        return dequantize_V_q1_0<T, ne>;
    ...
    } else if constexpr (type_V == GGML_TYPE_TURBOQ5_0) {
        return dequantize_V_turboq5_0<T, ne>;
    } else if constexpr (type_V == GGML_TYPE_TURBOQ6_0) {
        return dequantize_V_turboq6_0<T, ne>;
