...
static void set_rows_cuda_turboq4(
    ...
    // InnerQ: check/finalize calibration before kernel launch
    turbo_innerq_check_finalize(QK_TURBOQ4, ne00);
...
// Steps 1-4 (load, InnerQ calibrate/apply, parallel L2 norm, normalize, forward WHT) are
...
static void set_rows_cuda_turboq8(
    ...
    // InnerQ: check/finalize calibration before kernel launch
    turbo_innerq_check_finalize(QK_TURBOQ8, ne00);
...
// k_set_rows_oscar_int2_cal: CALIBRATED OSCAR 2-bit KV encode (InnerQ-fused, Track 1A / TODO 243).
// Identical to k_set_rows_oscar_int2 EXCEPT: a per-channel InnerQ scale diag(s) is applied to K BEFORE
// the Hadamard rotation (outlier-aware equalization). The inverse scale_inv=1/s is applied to Q at decode
// (graph-side ggml_mul in llama-graph.cpp) so dot(H·diag(1/s)·Q, H·diag(s)·K) = dot(Q,K). The channel
// index is j = position within the head (0..D-1); requires D==128 so j < INNERQ_MAX_CHANNELS. During the
// calibration phase (d_innerq_calibrating) per-channel K² is accumulated and K is stored with identity
// scale (d_innerq_active==0); after finalize the calibrated scale is applied. Mirrors the Turbo InnerQ
// accumulate/apply pattern (set-rows.cu k_set_rows_turboq3 ~line 395).
template<typename idx_t>
static __global__ void k_set_rows_oscar_int2_cal(
        const float * __restrict__ src0,
        const idx_t * __restrict__ src1,
        block_kv_oscar_int2 * __restrict__ dst,
        const int64_t ne00,
    ...
    const int j = threadIdx.x; // 0..D-1 (== channel within head; D==128 for the calibrated path)
    const int D = (int)blockDim.x; // = 128 (calibrated path is gated to head_dim==128 on the host)
    ...
    // Step 1: Load D elements for this group into shared memory.
    __shared__ float x[256];
    x[j] = src_row[i_group * D + j]; // i_group * D = offset into combined GQA row
    __syncthreads();

    // InnerQ calibration (on the ORIGINAL, pre-rotation values). Channel = j.
    if (d_innerq_calibrating) {
        atomicAdd(&d_innerq_sq_accum[j], x[j] * x[j]);
        if (j == 0) atomicAdd(&d_innerq_count, 1);
    }
    // Apply the calibrated per-channel scale BEFORE the Hadamard (identity until finalize).
    if (d_innerq_active) {
        x[j] *= d_innerq_scale[j];
    }
    __syncthreads();

    // Step 2: Full-dim D-pt WHT (standard Hadamard, D==128 → 7 stages).
    for (int h = 1; h < D; h <<= 1) {
        if ((j / h) % 2 == 0) {
            float a = x[j], b = x[j + h];
            x[j]     = a + b;
    ...
    // Steps 3-6: Per-sub-block min/max reduction and quantize (D==128 → single sub-block).
    const int ib = j / QK_OSCAR_INT2;
    const int jb = j % QK_OSCAR_INT2;
    const int n_warps_per_sb = QK_OSCAR_INT2 / WARP_SIZE;
    ...
    const int blk_base = (int)i_group * (D / QK_OSCAR_INT2);
    const uint8_t my_q = (uint8_t)(q & 0x3);
    const int lane = jb % WARP_SIZE;
    const uint8_t q1 = __shfl_sync(0xffffffff, my_q, lane ^ 1, WARP_SIZE);
    const uint8_t q2 = __shfl_sync(0xffffffff, my_q, lane ^ 2, WARP_SIZE);
...
static void set_rows_cuda_oscar_int2(
    ...
    if (ne01 > 0) {
        ...
        // Calibrated OSCAR INT2 (InnerQ-fused, Track 1A / TODO 243): armed by OSCAR_INNERQ/TURBO_INNERQ.
        // The per-channel index runs 0..wht_group-1 (= position within head, since wht_group==head_dim for
        // OSCAR's targets), so it must be < INNERQ_MAX_CHANNELS. Supported for head_dim ∈ {128, 256}
        // (OSCAR's primary target is head_dim==256, e.g. Qwen3.5-9B). When the env is unset,
        // turbo_innerq_check_finalize() no-ops and we run the plain kernel (byte-identical to the shipped
        // uncalibrated codec).
        bool use_cal = false;
        if ((wht_group == QK_OSCAR_INT2 || wht_group == OSCAR_WHT_FULL_DIM) && wht_group <= INNERQ_MAX_CHANNELS) {
            turbo_innerq_check_finalize((int)wht_group, ne00); // inits from env; finalizes when ready; no-op if disabled
            use_cal = turbo_innerq_is_enabled();
        }

        if (use_cal) {
            k_set_rows_oscar_int2_cal<idx_t><<<(int)ne_total, (int)wht_group, 0, stream>>>(
