...
static void rope_neox_cuda(const T *            x,
    ...
    if (use_fp64) {
        ggml_cuda_kernel_launch(rope_neox<forward, false, true, T, D>, launch_params,
            x, dst, ne00, ne01, ne02, s01, s02, s03, s1, s2, s3, n_dims, pos, freq_scale, ext_factor,
            attn_factor, corr_dims, theta_scale, nullptr, row_indices, set_rows_stride);
    } else if (freq_factors == nullptr) {
        ggml_cuda_kernel_launch(rope_neox<forward, false, false, T, D>, launch_params,
            x, dst, ne00, ne01, ne02, s01, s02, s03, s1, s2, s3, n_dims, pos, freq_scale, ext_factor,
            attn_factor, corr_dims, theta_scale, nullptr, row_indices, set_rows_stride);
    } else {
        ggml_cuda_kernel_launch(rope_neox<forward, true, false, T, D>, launch_params,
            x, dst, ne00, ne01, ne02, s01, s02, s03, s1, s2, s3, n_dims, pos, freq_scale, ext_factor,
            attn_factor, corr_dims, theta_scale, freq_factors, row_indices, set_rows_stride);
    }
}
