 ...
 *
 * Optimization log (10 versions tested on Qwen2.5-7B WHT4_0, RTX 5090):
...
static __global__ void mul_mat_vec_wht4_0_fused(
...
static __global__ void mul_mat_vec_wht3_0_fused(
        const void  * __restrict__ vx,
        const float * __restrict__ vy_rot,
        float       * __restrict__ dst,
        const int ncols_x,
...
void ggml_cuda_mul_mat_vec_tq(ggml_backend_cuda_context & ctx,
...

    {
        ...
        if (src0->type == GGML_TYPE_WHT4_0) {
            mul_mat_vec_wht4_0_fused<<<grid, block, 0, stream>>>(src0_d, d_act_rotated, dst_d, ncols_x, nrows_x);
        } else {
            mul_mat_vec_wht3_0_fused<<<grid, block, 0, stream>>>(src0_d, d_act_rotated, dst_d, ncols_x, nrows_x);
        }
    }
}
