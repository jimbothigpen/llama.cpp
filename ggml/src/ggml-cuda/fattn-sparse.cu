// fattn-sparse.cu — ggml CUDA/HIP backend dispatch for GGML_OP_FLASH_ATTN_SPARSE.
//
...
//
// HIP compat: uses nv_bfloat16 (aliased to __hip_bfloat16 under HIP via vendors/hip.h),
// ggml_cuda_pool_alloc<T> RAII instead of cudaMallocAsync/cudaFreeAsync.
...
void ggml_cuda_flash_attn_sparse(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    // Allocate pFlash-layout BF16 buffers via pool (RAII, HIP-compatible).
    ggml_cuda_pool_alloc<nv_bfloat16> Q_pf_alloc(ctx.pool(), Q_n);
    ggml_cuda_pool_alloc<nv_bfloat16> K_pf_alloc(ctx.pool(), K_n);
    ggml_cuda_pool_alloc<nv_bfloat16> V_pf_alloc(ctx.pool(), K_n);
    ggml_cuda_pool_alloc<nv_bfloat16> O_pf_alloc(ctx.pool(), O_n);

    nv_bfloat16 * Q_pf = Q_pf_alloc.get();
    nv_bfloat16 * K_pf = K_pf_alloc.get();
    nv_bfloat16 * V_pf = V_pf_alloc.get();
    nv_bfloat16 * O_pf = O_pf_alloc.get();
    ...
    if (K->type == GGML_TYPE_F16) {
        ...
        K_f16_alloc.alloc(ctx.pool(), K_n);
        to_fp16_k(K->data, K_f16_alloc.get(), K_n, stream);
        K_f16_src = K_f16_alloc.get();
    }
    ...
    if (V->type == GGML_TYPE_F16) {
        ...
        V_f16_alloc.alloc(ctx.pool(), K_n);
        to_fp16_v(V->data, V_f16_alloc.get(), K_n, stream);
        V_f16_src = V_f16_alloc.get();
    }
    k_f16_to_bf16_transpose_sh<<<(K_n + block - 1) / block, block, 0, stream>>>(
    ...
    // Pool allocs (Q_pf, K_pf, V_pf, O_pf, K_f16_alloc, V_f16_alloc) freed by RAII dtors.
}
