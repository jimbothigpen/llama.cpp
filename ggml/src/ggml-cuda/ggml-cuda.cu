...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (hint == GGML_HINT_SRC0_IS_HADAMARD) {
        ...
        // Fall through to regular matmul if FWHT not applicable (non-contiguous or unsupported size).
    }
        ...
        case GGML_OP_SET_ROWS:
            {
                ...
                // turboq2/turboq4 block size is 128, so head_dim must be divisible by 128
                if ((op->type == GGML_TYPE_TURBOQ2_0 || op->type == GGML_TYPE_TURBOQ4_0) && op->src[0]->ne[0] % 128 != 0) {
                    return false;
                }
                ...
                if ((op->type == GGML_TYPE_RQ_PLANAR3_0 || op->type == GGML_TYPE_RQ_PLANAR4_0 ||
