...
>
> **IQ3_KT — Status: Stable** — CPU, CUDA/HIP, and Vulkan backends; PPL healthy (9.05 on Qwen3.5-9B, +23.5% vs IQ3_K — inherent to single-codebook design; cross-backend PPL parity not yet formally verified).
>
...
> **IQ3_KT Vulkan status.** Shaders are tracked in git and pipelines are registered; the Vulkan path is code-complete. A formal cross-backend PPL parity gate (Vulkan vs. CPU/ROCm) has not yet been run for IQ3_KT. The `BACKEND_PARITY.md` row for IQ3_KT predates the landing and is stale — it still says "not yet ported." IQ2_KT and IQ4_KT Vulkan are fully deployed with PPL measurements confirming functional parity.
