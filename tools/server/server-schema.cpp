...
std::vector<std::unique_ptr<field>> make_llama_cmpl_schema(const common_params & params_base, task_params & params) {
    ...
    // Fork divergence (was server_task::params_from_json_cmpl, relocated here by mainline
    // #24150): allow per-request adjustment of the draft-model speculative parameters
    // (n_max/n_min/p_min). Upstream keeps these disabled "to keep things simple"; our serving
    // path enables them. The speculative *type* stays fixed at server startup — clients may only
    // assert it via the "spec_type" field below, never change it per request.
    ...
    // Fork divergence: "spec_type" is a validation assertion, not a setter. The server's
    // speculative type is configured at startup (--spec-type) and cannot change per request;
    // this lets a client assert the expected type (e.g. DFlash) and get an error on mismatch.
    add((new field_str("spec_type"))
        ->set_desc("Assert the server's active speculative decoding type; errors if not configured")
        ->set_handler([](field_eval_context & ctx, const json & data) {
            const std::string requested = data.at("spec_type").get<std::string>();
            if (requested.empty()) {
                return;
            }
            ...
            for (const auto & rt : requested_types) {
                if (std::find(ctx.params.speculative.types.begin(), ctx.params.speculative.types.end(), rt)
                        != ctx.params.speculative.types.end()) {
                    found = true;
                    break;
                }
            }
            if (!found) {
                throw std::invalid_argument("spec_type '" + requested +
                    ...
                    common_speculative_type_name_str(ctx.params.speculative.types) + ")");
            }
        }));

#if 0
    // Disabled (upstream + fork): per-request speculative type / ngram tuning. Our fork
    // deliberately does not allow changing the speculative type per request (see spec_type above).
