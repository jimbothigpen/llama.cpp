...
### Fixed — EAGLE3 fc_norm wire + DFlash n_ubatch auto-set (TODO 255, 256) (2026-06-27)

`c6d174e8c`. Two loader-side bug fixes for spec-decode:

**EAGLE3 fc_norm** (TODO 255): the EAGLE3 draft-model converter did not wire the
optional `fc_norm` layer present in some EAGLE3 checkpoints (e.g. `BLR2/Eagle3-Qwen3.5-9B`).
Draft models with `fc_norm` loaded silently but produced degraded accept rates. The
converter now copies `fc_norm` when present; heads without it are unaffected.

**DFlash n_ubatch auto-set** (TODO 256): DFlash spec-decode requires
`n_ubatch ≥ n_draft + 1`, but prior code let users set `n_ubatch < n_draft`,
triggering `SIGABRT rc=134` at inference time. The loader now auto-adjusts
`n_ubatch` upward when DFlash is active, with a warning log.

Both fixes verified on Qwen3.5-9B EAGLE3 with DFlash; `rc=134` on
`--spec-type eagle3 --spec-dflash` resolved.

Changed: spec-decode loader + converter (~30 LOC).

### Added — WQ3 TCQ, WHT5/6/8 weight quants, turboq5/6 KV types (reship wave 2026-06-22/23)

`5568b37bb` (reship-wave-integration merge). Four related feature groups:

**WQ3 TCQ (trellis-coded 3-bit weight quant, TODO 259-Ph4):** slot 92. Applies the
FWHT trellis quantizer to model weights (not KV cache), with L=10 codeword length.
Full 4-backend coverage: CPU (`949d3eecd`), CUDA/HIP (`56918b30e`), Vulkan. Attribution:
TCQ algorithm from TheTom.

**WHT5/6/8 (FWHT 5/6/8-bit weight quants):** slots 82/83/84 (`4fd735f2c` +
`560aec6aa`). Extends the FWHT weight-quant ladder into 5/6/8-bit territory; CPU +
GPU backends wired.

**turboq5/6 KV types (slots 64/65, TODO 250 sub-2/3):** FWHT + uniform-grid 5/6-bit
KV cache types (`ed8962b5d`). Dormant: PPL-equivalent to peer bpw types; no TPS
advantage. Enums wired; not in default KV ladder.

**Specdecode fixes + fwd-sync-converge**: companion bugfixes and mainline convergence
for the reship wave.

Changed: `ggml/src/ggml-quants.{h,c}`, `ggml/src/ggml-cuda/`, Vulkan shader + dispatch,
`src/llama.cpp` (quant registration) (~400+ LOC net).
